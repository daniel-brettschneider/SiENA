...
PLC_Mac::PLC_Mac ()
{
	...
	m_macMinBE = MyConfig::Get()->getInt("m_macMinBE"); //1;
	m_macMaxBE = MyConfig::Get()->getInt("m_macMaxBE"); //3;
	m_macMaxCSMABackoffs = MyConfig::Get()->getInt("m_macMaxCSMABackoffs"); //10
	m_aUnitBackoffPeriod = MyConfig::Get()->getInt("m_aUnitBackoffPeriod"); //20 symbols


	m_ackPacket = 0;
	m_rxPacket = 0;
	m_txQueue = CreateObject<DropTailQueue> ();
//	m_nextPacket = UINT64_MAX;
}
...
{
	m_aUnitBackoffPeriod = MyConfig::Get()->getInt("m_aUnitBackoffPeriod");
...
{
	PLC_MAC_FUNCTION (this);
	PLC_MAC_INFO ("Received packet (Uid: " << p->GetUid() << ") " << *p);
...
PLC_Mac::TriggerTransmission (void)
{
	...
	// clean
	if(Simulator::Now().GetMilliSeconds() > 800) {
		Time t = Simulator::Now() - MilliSeconds(800);
		while(uids.size() > 0) {
			if(uids.front().first.GetMicroSeconds() < t.GetMicroSeconds())
	...
	// check
	bool found = false;
	std::vector<std::pair<Time, uint64_t> >::iterator it;
	for(it = uids.begin(); it != uids.end(); ++it) {
		if(it->second == p->GetUid()) {
			found = true;
			break;
		}
	}
	if(!found) {
		if (GetPhy ()->InitializeTx (p) == false) {
			...
			if(!isFragment)
				uids.push_back(std::pair<Time, uint64_t>(Simulator::Now(), p->GetUid()));
		}

	}
	else
		PLC_MAC_LOGIC ("Duplicate transmission rejected");
}
...
PLC_ArqMac::DoProcess (Ptr<const Packet> p)
{
	...
	PLC_MAC_LOGIC ("Received packet (Uid: " << p->GetUid() << ") " << *p);
...
PLC_ArqMac::NotifyTransmissionEnd (void)
{
	...
	if (m_ackPacket)
	{
		...
		m_ackPacket = 0;
		if (m_txQueue->IsEmpty () == false) {
//			Ptr<Packet> tmp1 = m_txQueue->Peek();
//			uint64_t tmp = tmp1->GetUid();
//			if (tmp != m_nextPacket)
//			{
				PLC_MAC_LOGIC ("Initializing CSMA/CA for next frame to be sent...");
//				m_nextPacket = tmp;
				//std::cerr << "\t" << Simulator::Now() << " next packet: " << m_nextPacket << std::endl;
				InitializeCsmaCa ();
//			}
		}
//		if (m_txQueue->IsEmpty () == false)
...
PLC_ArqMac::AcknowledgementTimeout(void)
{
	...
	if (m_txQueue->IsEmpty () == false)
	{
		...
		else
		{
			...
			Ptr<Packet> p = m_txQueue->Dequeue()->GetPacket();
			m_replays = 0;
...
PLC_HarqMac::DoProcess (Ptr<const Packet> p)
{
	...
	if (dst_addr == m_address)
	{
		...
		{
			Ptr<Packet> lastTxPacket = m_txQueue->Peek()->GetPacket();
			PLC_MacHeader txMacHdr;
			lastTxPacket->PeekHeader(txMacHdr);
...
PLC_HarqMac::NotifyTransmissionEnd (void)
{
	...
	{
		// Data frame sent
		Ptr<Packet> p = m_txQueue->Peek()->GetPacket();
		NS_ASSERT_MSG (p, "No packet in send queue, something went wrong!");
